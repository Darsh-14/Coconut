...
await page.goto(`${BASE}/disputes`, { waitUntil: 'networkidle' })
await page.locator('table tbody tr').first().waitFor({ timeout: 30000 })
const caseLink = page.locator('table tbody a[href^="/disputes/disp"]').first()
...
await page.goto(`${BASE}/disputes/${target}`, { waitUntil: 'networkidle' })
...
if (contest) {
  await page.goto(`${BASE}/disputes/${contest.dispute_id}`, { waitUntil: 'networkidle' })
...
for (const width of [1440, 1280, 1024, 900]) {
  ...
  await p2.goto(`${BASE}/disputes`, { waitUntil: 'networkidle' })
  await p2.locator('table tbody tr').first().waitFor({ timeout: 30000 })

  const navCount = await p2.locator('a[href="/"]:visible, a[href="/disputes"]:visible, a[href="/metrics"]:visible').count()
...
await light.goto(`${BASE}/disputes`, { waitUntil: 'networkidle' })
